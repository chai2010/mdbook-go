...
import (
	...
	"github.com/chai2010/mdbook-go/pkg/mdbook"
)

//go:embed tmpl/page.html
var tmplPage string

func (p *BookRendor) run(book *mdbook.Book) (err error) {
...
func (p *BookRendor) init(book *mdbook.Book) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("%v", r)
		}
...
func (p *BookRendor) buildSidebarItem(idx int, item mdbook.SummaryItem, sidebarNumbers []string) *SidebarItem {
...
func (p *BookRendor) buildPageInfo(idx int, item mdbook.SummaryItem) *PageInfo {
	page := new(PageInfo)
...
func (p *BookRendor) getPageLocalAbsPath(item mdbook.SummaryItem) string {
	return filepath.Clean(filepath.Join(p.Book.Root, p.Book.Info.Book.Src, item.Location))
}

func (p *BookRendor) getPageRelPath(item mdbook.SummaryItem) string {
...
func (p *BookRendor) getPageRootPath(item mdbook.SummaryItem) string {
...
func (p *BookRendor) getPageTitle(item mdbook.SummaryItem) string {
	return fmt.Sprintf("%s - %s", item.Name, p.BookInfo.Title)
}

func (p *BookRendor) loadPageContent(item mdbook.SummaryItem) string {
